...
HFONT UV_CreateFont(char *fx,int szx,int bold)
{
    ...
    ReleaseDC(NULL, hdc);
    hfx = CreateFont(lfHeight, 0, 0, 0, bold, 0, 0, 0, 0, 0, 0, 0, 0,(LPCSTR)fx);
    return hfx;
}
...
BOOL CALLBACK UV_AboutDialogProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
    {
        ...
        if(BeginPaint(hwnd, &ps))
        {
            ...
            __UV__DrawScrollText(hdcMem, (LPTSTR)UV.STEXT,UV.sfont,UV.FONT_COLOR,UV.WX,UV.WY, UV.BW, UV.BH);
