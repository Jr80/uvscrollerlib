                          ...
                          INT nLeft, INT nTop, INT nRight, INT nBottom)
{
    ...
    DrawText(hdcDest, lpszText, strlen((const char *)lpszText), &rcText, UV.TEXT_ALIGNMENT);
}
...
{
	SetWindowLong(hWnd, GWL_EXSTYLE, GetWindowLong(hWnd, GWL_EXSTYLE) | WS_EX_LAYERED);
	SetLayeredWindowAttributes(hWnd,RGB(0,0,0), trans, LWA_ALPHA);
}
...
void __UV__SetRegion(HWND hWnd,int RGNID)
{
    ...
    hRsrc = FindResource(hInst, MAKEINTRESOURCE(RGNID), (LPCSTR)"RGN");
